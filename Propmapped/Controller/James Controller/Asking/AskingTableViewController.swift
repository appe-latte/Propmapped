...
import UIKit
/*
class AskingTableViewController: UITableViewController {
    ...
    func setUI() {
        self.navigationItem.title = "Av. ASKING \(self.postCode)";
    ...
    func loadData() {
        ...
        let api = AskingPriceAPI();
        api.submitRequest(postCode:"B37") { (data) in
            if let asking = data {
                self.askingData = asking;
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if let count = self.askingData?.data?.rawData?.count {
            return count;
        }
        return 0;
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.headLine.text = "";
        cell.subLine.text = "";
        if let property = self.askingData?.data?.rawData?[indexPath.row] {
...
extension AskingTableViewController {
    
    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        
        if section == 0 {
            return "Average Asking Prices"
        }
        return ""
    }
